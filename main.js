...
const { queenmd } = require('./lib/system');
...
function queenLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [queen-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
...
queenLog(`Loading ${pluginFiles.length} plugins...`, 'info');
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                queenLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            queenLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            queenLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                queenLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { queenLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                queenLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                } catch (e) { queenLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                queenLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function queenPair(number, res = null) {
    ...
    try {
        ...
        if (!existingSession) {
            queenLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                queenLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            queenLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }

        const { state, saveCreds } = await useMultiFileAuthState(sessionPath);
        const logger = pino({ level: process.env.NODE_ENV === 'production' ? 'fatal' : 'debug' });

        const queenStore = createqueenStore();
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await queenStore.loadMessage(key.remoteJid, key.id);
        ...
        if (!conn.authState.creds.registered) {
            ...
            try {
                ...
                queenLog(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
        ...
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, queenStore);
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                await queenmd(conn);
                queenLog(`Connected: ${sanitizedNumber}`, 'success');
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© queen-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:queen-MD BOY\nORG:queen-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© queen-MD' } }
                    }},
        ...
        queenLog(`queen-MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'queen-md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 queen-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        ...
        queenLog('Auto-reconnect completed', 'success');
